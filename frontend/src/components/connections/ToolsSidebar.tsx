...
export function ToolsSidebar({ onToolDragStart, onClose: _onClose }: ToolsSidebarProps) {
  ...
  return (
    <motion.div
      className="w-80 h-full flex flex-col min-w-0"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="p-4">
        <motion.div className="relative" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          ...
          <input
            ...
            className="neu-search-input w-full pl-10 pr-4 py-2.5 rounded-xl text-[var(--color-text-primary)]"
          />
        </motion.div>
      </div>

      {/* Scrollable content area */}
      <div className="theme-scrollbar flex-1 overflow-y-auto px-4 pb-4 min-h-0">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {filteredTools.slice(0, 9).map((tool, i) => (
              <motion.div
                key={`rec-${tool.id}`}
                className="neu-tool-card group flex items-center justify-center aspect-square p-3 cursor-grab active:cursor-grabbing"
                title={tool.name}
        ...
        {Object.entries(groupedTools).map(([category, tools]) => (
          <motion.div key={category} className="mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
            <h3 className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider mb-3">
            ...
            <div className="space-y-2">
              {tools.map((tool) => (
                <motion.div
                  key={tool.id}
                  className="neu-tool-card group flex items-center gap-3 px-3 py-2.5 cursor-grab active:cursor-grabbing"
                  whileHover={{ x: 2 }}
                  whileTap={{ scale: 0.98 }}
                ...
                >
                  ...
                  {/* Arrow */}
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--color-text-muted)] group-hover:text-[#B5BAC1] transition-colors">
