...
export function ToolsSidebar({ onToolDragStart, onClose }: ToolsSidebarProps) {
  ...
  const getToolIcon = (icon: string, size: 'sm' | 'md' = 'md') => {
  ...
  return (
    <div className="w-80 h-full border-l border-[var(--color-border)] bg-[var(--color-bg-secondary)] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {/* Recommended Section - 3x3 Grid with Icons Only */}
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {filteredTools.slice(0, 9).map((tool) => (
              <div
                ...
                className="group flex items-center justify-center aspect-square p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-tertiary)] cursor-grab active:cursor-grabbing hover:border-[var(--color-accent)] hover:bg-[var(--color-bg-hover)] transition-all duration-200"
                title={tool.name}
              >
                <div className="text-[var(--color-text-secondary)] transition-all duration-200 group-hover:text-[var(--color-accent)] group-hover:scale-110">
            ...
            <div className="space-y-2">
              {tools.map((tool) => (
                <div
                  ...
                  draggable
                  onDragStart={handleDragStart(tool)}
                  className="group flex items-center gap-3 px-3 py-2.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-tertiary)] cursor-grab active:cursor-grabbing hover:border-[var(--color-accent)] hover:bg-[var(--color-bg-hover)] transition-all duration-200"
                >
                  {/* Icon */}
                  <div className="flex h-8 w-8 items-center justify-center flex-shrink-0 text-[var(--color-text-secondary)] transition-all duration-200 group-hover:text-[var(--color-accent)]">
                  ...
                  {/* Arrow */}
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--color-text-muted)] group-hover:text-[var(--color-accent)] transition-colors">
